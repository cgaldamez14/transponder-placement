 ...
 * a list of virtual nodes and virtual links. The user must specify the max number of nodes a virtual
 * request can have. The assumption for these virtual request is that they follow a linear array 
 * topology meaning that the number of virtual links is always going to be one less than the number
 * of virtual nodes. Each virtual node will have a bandwidth demand , and each virtual node will have
 * a computational demand and a requested function. If a request cannot be mapped to a physical topology
...
public class VirtualRequest {
	...
	/**
	 * Constructor creates the virtual nodes and virtual links for the virtual request being created
	 * @param maxNodes - Max number of nodes the virtual request can have
	...
	public VirtualRequest(int maxNodes, NetworkTopology type) {
		...
		// ------------------------ Map source node and destination node randomly ------------------------ //
		...
		for (int virtualNode = 0; virtualNode < numberOfVirtualLinks; virtualNode++)
			virtualLinks.add(new VirtualLink(new Pair<Integer, Integer>(virtualNodes.get(virtualNode).getID(), 
									 									virtualNodes.get(virtualNode + 1).getID()), 
							 Link.generateRandomBandwidth()));
	}
